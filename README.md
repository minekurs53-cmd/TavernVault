...
- **程序内编辑**
  ...
  - 预设：**可视化视图**（只读，B 计划一期）——采样参数中文总览、按 prompt_order 解析的生效顺序（启用状态/角色/字数）、未排序提示词清单、点击查看提示词全文与统计；原文视图仍可直接编辑保存
...
- **备份与还原**：所有覆盖写入（编辑保存/还原/重命名）前自动备份原文件到 `%APPDATA%\TavernVaultackups`；详情页可查看备份列表、一键还原（还原前同样先备份当前）、删除；每文件保留份数与开关在库设置中调整
...
│  ├─ Storage/     # 设置/索引持久化 + 查询(Vault) + 备份存储(BackupStore)
│  ├─ Detection/   # 基于内容的类型识别
│  ├─ Scanning/    # 库扫描与索引构建
...
后续方向（按优先级）：
1. **接入酒馆项目**（可行性已确认，见 `docs/st-sync-feasibility.md`）：接入向导自动注册两个酒馆的数据目录为带标记的库根（实测本机 SillyTavern → `data\default-user`，TauriTavern → `cache\default-user`，均为标准布局），配合已完成的自动备份实现"局外编辑、酒馆内生效"；护栏：酒馆源内默认禁止移动/重命名角色卡（聊天记录按文件名引用）
2. **预设可视化二期/三期**：启用/禁用开关与采样参数编辑（写 prompt_order）、拖拽排序、新增提示词
3. 内嵌世界书合入（从独立世界书导入到卡片）
4. 重复资源检测（按内容指纹）
