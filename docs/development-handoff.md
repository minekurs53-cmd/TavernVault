...
- 验证情况：Release 构建 0 警告 0 错误；单测 97/97 + preset-model 18/18 + 集成 14/14；冒烟 **214 项 × 2 轮全绿**；
...
1. **便携模式（可移植性缩减版，v0.7.5 奥卡姆复审）**：首启向导**砍掉**——空库空态引导 + 收纳入库 +
   库设置已完整覆盖首启路径，向导属重复建设；只保留 `--portable` 参数（数据目录随程序目录而非
   %APPDATA%），服务"拷贝即用"的分发/换机场景。
3. **README 重写** ✅ v0.7.5 完成（结构化分区 + 维护约定注释，"新功能去哪写描述"有了明确答案；
   约定全文见 quick-reference「文档维护约定」）。
...
- [x] **内嵌世界书合入**（v0.7.6，见 §3.22）：独立书 → 卡片内嵌书，与导出互为反向。
...
**文档版本**：4.8 · **最后更新**：2026-09-05 · 对应程序版本 v0.7.6
