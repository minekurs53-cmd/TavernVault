...
4. **GitHub 直连失败**（国内网络）：本仓库已配置 `git config http.proxy http://127.0.0.1:7897`（用户系统代理）。推送前若报 `Failed to connect to github.com`，确认代理在监听 7897。
...
- [ ] 提交前 `git status` 确认分支；推送走代理 7897
