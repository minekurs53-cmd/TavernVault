...
| v0.7.4 | 工程底线：**API 集成测试进 dotnet test**（真实 Kestrel + 隔离临时库，14 项核心合同回归，永不触碰真实用户库）+ **GitHub Actions CI**（windows runner：构建 → 全部测试 → 冒烟）；冒烟与集成测试分工明确（全量穷举 vs 核心合同） |

后续方向（按优先级，2026-09 依真实使用反馈复审）：
1. 内嵌世界书合入（从独立世界书导入到卡片；导出已支持）
2. **可移植性**：首次启动向导（一键创建我的库 + 探测酒馆，收纳能力已就绪）、可选便携模式
3. 前端 editor.js 拆分 + UI 自动化安全网
4. **README 重写**（提案已定，等确认结构后执行）
5. 应用图标设计（已入队：统一窗口/exe .ico 与 favicon，现为占位图标）
6. 重复资源检测（按内容指纹）、内容指纹追踪用户数据——推迟

已砍掉（复审存档）：
- ~~酒馆接入增强（断链提示/子目录白名单）~~ → v0.7.1 复审砍掉：就地编辑退役后酒馆库只读+导出，断链风险场景消失，白名单随模板目录回撤失去意义

已完成（历史路线存档）：
- ~~API 集成测试进 dotnet test + CI~~ ✅ v0.7.4 完成（集成 14 项核心合同 + GitHub Actions 全自动）
