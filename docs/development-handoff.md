...
1. **FileSystemWatcher 自动重扫【上调为下一迭代首选】**：真实使用最先撞到的痛点——外部/酒馆侧的改动
   必须手动"重新扫描"才可见。监听已登记库根（防抖 + 增量 UpsertItem/RemoveItem）；
   配套的批量操作（多选移动/打标）推迟。
2. **API 集成测试进 `dotnet test` + CI**：TestServer 收编冒烟，摆脱"手工起服务再跑脚本"两步走；
   开源前的工程底线。
3. **内嵌世界书合入**：独立世界书导入卡片内嵌书（导出已支持）。纯局外操作，不受酒馆问题影响，
   用户价值明确。
4. **可移植性**（v0.4.3 起步）：首次启动向导（无库根时引导选目录/探测酒馆）、可选便携模式——
   打包分发已落地（§3.16），向导是分发体验的下一环。
...
**文档版本**：4.3 · **最后更新**：2026-09-05 · 对应程序版本 v0.7.1
