...
- 分支 `qoder/TavernVault`；v0.7.2 已推送（`8766a4f`），v0.7.3 开发完成（收纳入库）。
- **项目已进入真实使用阶段**（用户自用 ST/TT 各 70+ 资源）：需求与优先级以真实使用反馈为准。
- 验证情况：Release 构建 0 警告 0 错误；单测 93/93 + preset-model Node 测试 18/18；冒烟 **207 项 × 2 轮全绿**；
...
1. **内嵌世界书合入**：独立世界书导入卡片内嵌书（导出已支持）。纯局外操作，不受酒馆问题影响，
   用户价值明确。
2. **便携模式（可移植性缩减版，v0.7.5 奥卡姆复审）**：首启向导**砍掉**——空库空态引导 + 收纳入库 +
...
**文档版本**：4.7 · **最后更新**：2026-09-05 · 对应程序版本 v0.7.5
