...
> **修复状态（2026-09-02）**：本报告的以下条目已在 v0.5.1 修复并验证（构建 0 警告、单测 49/49、同数据目录冒烟 3 轮 81/81 全绿）：**P0-1 XSS**、**P1-1**（设置损坏防护+index.bak+自愈条件）、**P1-5**（ReparsePoint）、**P1-6**（Title 清洗+SanitizeFileName）、**P1-9**（缩略图随数据目录+旁车失效键）、**N1**（还原自逐出+原子写回）、**N2**（冒烟清理+可重复）、**N3**（Mutex 所有权+按数据目录隔离）；另顺带修复 P1-4（manifest 原子写）、备份/缩略图目录写前自愈、List 按磁盘过滤幽灵条目、扫描 tmp 前缀过滤、`ResolveDataDir` 绝对路径化。其余 P2 与 v0.5.2/v0.6 计划仍以本报告为权威清单（见 §8 路线图）。
