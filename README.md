...
1. **FileSystemWatcher 自动重扫**（优先级上调）：外部改动目前需手动"重新扫描"，真实使用中最先撞到；批量操作（多选移动/打标）推迟
2. 内嵌世界书合入（从独立世界书导入到卡片；导出已支持）——纯局外操作，不受酒馆同步问题影响
3. **可移植性**：首次启动向导（开箱即配库根）、可选便携模式——打包分发已落地，向导是分发体验的下一环
4. API 集成测试进 `dotnet test`（TestServer 收编冒烟）+ CI——开源前的工程底线
