...
1. **收纳入库**（v0.7.3 首选）：选一个散乱文件夹 → 按内容识别分类预览 → 批量复制进局外库根的类型子目录（源不动、报告清单）——让"摆得没这么井井有条"的机器也能一键建库
2. API 集成测试进 `dotnet test`（TestServer 收编冒烟）+ CI——开源前的工程底线
3. 内嵌世界书合入（从独立世界书导入到卡片；导出已支持）
4. **可移植性**：首次启动向导（一键创建我的库 + 探测酒馆，与收纳入库联动）、可选便携模式
5. 前端 editor.js 拆分 + UI 自动化安全网
